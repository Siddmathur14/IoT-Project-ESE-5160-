...
int SHTC3_SendI2cCommand(uint8_t *buf, uint8_t size)
{
    ...
    sensorPacketWrite.data = (uint8_t *)SHTC3_WAKEUP_CMD;      // Requires a pointer to the data to send. Why dont we put a #define here?
...
int SHTC3_Read_Data(uint8_t *buf, uint8_t size)
{
	...
	return i2c_master_read_packet_job(&i2cSensorBusInstance, &sensorPacketWrite);
}
