...
QueueHandle_t xQueueTemp = NULL;
QueueHandle_t xQueueHeartrate = NULL;
...
// {
// // 	 if (strncmp((char *)msgData->topicName->lenstring.data, BUTTON_TOPIC, msgData->message->payloadlen) == 0) {
...
{	uint8_t buffer1[64];
	LogMessage(LOG_DEBUG_LVL, "\r\n Button topic received!\r\n");
    //LogMessage(LOG_DEBUG_LVL, "\r\n %.*s", msgData->topicName->lenstring.len, msgData->topicName->lenstring.data);
	//LogMessage(LOG_DEBUG_LVL, "\r\n %.*s", msgData->message->payload , msgData->message->payloadlen);
	//if (strncmp((char *)msgData->topicName->lenstring.data, IMU_TOPIC, msgData->message->payloadlen) == 0) {
		// if (strncmp((char *)msgData->message->payload, BUTTON_TOPIC_OFF, msgData->message->payloadlen) == 0) {
...
void SubscribeHandlerDataTopic(MessageData *msgData)
{
	LogMessage(LOG_DEBUG_LVL, "\r\nData sent to:\r\n");
    ...
    switch (type) {
        ...
        case MQTT_CALLBACK_CONNECTED:
            if (data->connected.result == MQTT_CONN_RESULT_ACCEPT) {
				...
				//mqtt_subscribe(module_inst, BUTTON_TOPIC, 1, SubscribeButtonTopic);
				//mqtt_subscribe(module_inst, , 1, SubscribeHandlerDataTopic);
